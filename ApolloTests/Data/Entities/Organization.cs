...
namespace ApolloTests.Data.Entity
{
    public class Organization:BaseEntity
    {
        ...
        {
            return new Organization((int)GetSQLService().executeQuery("SELECT TOP (1) org.Id FROM [party].[Organization] org " +
        ...
        public static String GetNextValidName(String name, out Mutex mutex)
        {
            ...
            var result = Main.ObjectContainer.Resolve<SQL>().executeQuery("SELECT [Name] FROM [party].[Organization] Where [Name] Like 'Automation API org%';");
        ...
        private static List<dynamic> TypeNames = GetRestAPIService().GET("/config/lookup/4").ToObject<List<dynamic>>();
