// ------------------------------------------------------------------------------
//  <auto-generated>
//      This code was generated by SpecFlow (https://www.specflow.org/).
//      SpecFlow Version:3.9.0.0
//      SpecFlow Generator Version:3.9.0.0
// 
//      Changes to this file may cause incorrect behavior and will be lost if
//      the code is regenerated.
//  </auto-generated>
// ------------------------------------------------------------------------------
#region Designer generated code
#pragma warning disable
namespace ApolloQAAutomation.Features.WorkmansComp.Issued.ServiceIndustries_I
{
    using TechTalk.SpecFlow;
    using System;
    using System.Linq;
    
    
    [System.CodeDom.Compiler.GeneratedCodeAttribute("TechTalk.SpecFlow", "3.9.0.0")]
    [System.Runtime.CompilerServices.CompilerGeneratedAttribute()]
    [Microsoft.VisualStudio.TestTools.UnitTesting.TestClassAttribute()]
    public partial class WC_IronGrillOrRailing_IFeature
    {
        
        private static TechTalk.SpecFlow.ITestRunner testRunner;
        
        private Microsoft.VisualStudio.TestTools.UnitTesting.TestContext _testContext;
        
        private static string[] featureTags = ((string[])(null));
        
#line 1 "WC_IronGrillOrRailing_I.feature"
#line hidden
        
        public virtual Microsoft.VisualStudio.TestTools.UnitTesting.TestContext TestContext
        {
            get
            {
                return this._testContext;
            }
            set
            {
                this._testContext = value;
            }
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.ClassInitializeAttribute()]
        public static void FeatureSetup(Microsoft.VisualStudio.TestTools.UnitTesting.TestContext testContext)
        {
            testRunner = TechTalk.SpecFlow.TestRunnerManager.GetTestRunner();
            TechTalk.SpecFlow.FeatureInfo featureInfo = new TechTalk.SpecFlow.FeatureInfo(new System.Globalization.CultureInfo("en-US"), "Features/Workman\'s Comp/Issued/ServiceIndustries_I", "WC_IronGrillOrRailing_I", "User Story 89461: WC | Feature File - Issued | SCENARIO 22 Ornamental Iron Grill " +
                    "or Railing Erection - MA", ProgrammingLanguage.CSharp, featureTags);
            testRunner.OnFeatureStart(featureInfo);
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.ClassCleanupAttribute()]
        public static void FeatureTearDown()
        {
            testRunner.OnFeatureEnd();
            testRunner = null;
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestInitializeAttribute()]
        public void TestInitialize()
        {
            if (((testRunner.FeatureContext != null) 
                        && (testRunner.FeatureContext.FeatureInfo.Title != "WC_IronGrillOrRailing_I")))
            {
                global::ApolloQAAutomation.Features.WorkmansComp.Issued.ServiceIndustries_I.WC_IronGrillOrRailing_IFeature.FeatureSetup(null);
            }
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCleanupAttribute()]
        public void TestTearDown()
        {
            testRunner.OnScenarioEnd();
        }
        
        public void ScenarioInitialize(TechTalk.SpecFlow.ScenarioInfo scenarioInfo)
        {
            testRunner.OnScenarioInitialize(scenarioInfo);
            testRunner.ScenarioContext.ScenarioContainer.RegisterInstanceAs<Microsoft.VisualStudio.TestTools.UnitTesting.TestContext>(_testContext);
        }
        
        public void ScenarioStart()
        {
            testRunner.OnScenarioStart();
        }
        
        public void ScenarioCleanup()
        {
            testRunner.CollectScenarioErrors();
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestMethodAttribute()]
        [Microsoft.VisualStudio.TestTools.UnitTesting.DescriptionAttribute("WC Iron Grill Or Railing creates issued policy for Massachusetts")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestPropertyAttribute("FeatureTitle", "WC_IronGrillOrRailing_I")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Issued")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Regression")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Staging")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("MA")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Construction")]
        public void WCIronGrillOrRailingCreatesIssuedPolicyForMassachusetts()
        {
            string[] tagsOfScenario = new string[] {
                    "Issued",
                    "Regression",
                    "Staging",
                    "MA",
                    "Construction"};
            System.Collections.Specialized.OrderedDictionary argumentsOfScenario = new System.Collections.Specialized.OrderedDictionary();
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("WC Iron Grill Or Railing creates issued policy for Massachusetts", null, tagsOfScenario, argumentsOfScenario, featureTags);
#line 6
this.ScenarioInitialize(scenarioInfo);
#line hidden
            if ((TagHelper.ContainsIgnoreTag(tagsOfScenario) || TagHelper.ContainsIgnoreTag(featureTags)))
            {
                testRunner.SkipScenario();
            }
            else
            {
                this.ScenarioStart();
                TechTalk.SpecFlow.Table table2758 = new TechTalk.SpecFlow.Table(new string[] {
                            "Industry",
                            "Employees",
                            "Location",
                            "Own or Lease",
                            "ZIP Code",
                            "LOB"});
                table2758.AddRow(new string[] {
                            "Ornamental Iron Grill or Railing Erection",
                            "3",
                            "I Lease a Space From Others",
                            "",
                            "02301",
                            "WC"});
#line 7
 testRunner.Given("user starts a quote with:", ((string)(null)), table2758, "Given ");
#line hidden
#line 10
 testRunner.Then("user verifies the appearance of the WC About You Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2759 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2759.AddRow(new string[] {
                            "When do you want your policy to start?",
                            ""});
                table2759.AddRow(new string[] {
                            "When did you start your business?",
                            "Started 8 years ago"});
                table2759.AddRow(new string[] {
                            "How is your business structured?",
                            "Partnership"});
                table2759.AddRow(new string[] {
                            "What is the total annual payroll for W-2 employees? (exclude business owners/offi" +
                                "cers)",
                            "50000"});
                table2759.AddRow(new string[] {
                            "Are there any employees that never travel to job sites or do any construction wor" +
                                "k?",
                            "no"});
                table2759.AddRow(new string[] {
                            "Do you use any subcontractors or pay any workers using IRS Form 1099?",
                            "no"});
                table2759.AddRow(new string[] {
                            "Business",
                            "Test refer"});
                table2759.AddRow(new string[] {
                            "Address",
                            "701 SE 71st St;Brockton"});
                table2759.AddRow(new string[] {
                            "Contact First Name",
                            "TestF"});
                table2759.AddRow(new string[] {
                            "Contact Last Name",
                            "TestL"});
                table2759.AddRow(new string[] {
                            "Email",
                            "Test@Test123.com"});
                table2759.AddRow(new string[] {
                            "Phone",
                            "1231233333"});
                table2759.AddRow(new string[] {
                            "Business website",
                            "test.com"});
#line 11
 testRunner.Then("user fills out the WC About You page with these values:", ((string)(null)), table2759, "Then ");
#line hidden
#line 26
 testRunner.Then("user verifies the appearance of the WC Owners and Officers Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2760 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2760.AddRow(new string[] {
                            "How many owners/officers does your business have?",
                            "5+"});
                table2760.AddRow(new string[] {
                            "How many owners/officers do you want to cover with this policy?",
                            "3"});
                table2760.AddRow(new string[] {
                            "How many owners/officers do you want to exclude from this policy?",
                            "2"});
#line 27
 testRunner.Then("user handles the WC OO kickoff questions with these values:", ((string)(null)), table2760, "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2761 = new TechTalk.SpecFlow.Table(new string[] {
                            "First Name",
                            "Last Name"});
                table2761.AddRow(new string[] {
                            "Kirst",
                            "Nast"});
                table2761.AddRow(new string[] {
                            "Burger",
                            "King"});
                table2761.AddRow(new string[] {
                            "HappyIs",
                            "Madden"});
#line 32
 testRunner.Then("user handles the WC Covered OO with these values:", ((string)(null)), table2761, "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2762 = new TechTalk.SpecFlow.Table(new string[] {
                            "First Name",
                            "Last Name"});
                table2762.AddRow(new string[] {
                            "OneF",
                            "OneL"});
                table2762.AddRow(new string[] {
                            "TwoF",
                            "TwoL"});
#line 37
 testRunner.Then("user handles the WC Excluded OO with these values:", ((string)(null)), table2762, "Then ");
#line hidden
#line 41
 testRunner.Then("user continues on from the WC OO page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2763 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2763.AddRow(new string[] {
                            "Do you install any structures such as barns, garages, gas stations, greenhouses, " +
                                "hangars, or sheds?",
                            "no"});
                table2763.AddRow(new string[] {
                            "Do you install any overhead doors?",
                            "no"});
                table2763.AddRow(new string[] {
                            "Do you ever transport six or more workers in the same vehicle?",
                            "no"});
                table2763.AddRow(new string[] {
                            "In the past 3 years how many Workers\' Compensation claims were reported?",
                            "None"});
                table2763.AddRow(new string[] {
                            "Do you perform any work over 30 feet above ground level?",
                            "no"});
                table2763.AddRow(new string[] {
                            "Do you do any demolition or wrecking of entire buildings or homes?",
                            "no"});
                table2763.AddRow(new string[] {
                            "Do the business owner(s) of this business have a combined majority ownership in a" +
                                "ny other construction business?",
                            "no"});
                table2763.AddRow(new string[] {
                            "Do you currently have a Workers\' Compensation insurance policy in effect?",
                            "no"});
                table2763.AddRow(new string[] {
                            "When was your last policy in effect?",
                            "Never no prior insurance"});
                table2763.AddRow(new string[] {
                            "Do you do any construction work in New York?",
                            "no"});
                table2763.AddRow(new string[] {
                            "Do you have multiple locations in more than one state?",
                            "no"});
                table2763.AddRow(new string[] {
                            "Do you do any work on barges/vessels/docks/or bridges over water?",
                            "no"});
#line 42
 testRunner.Then("user fills out the WC Your Services page", ((string)(null)), table2763, "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2764 = new TechTalk.SpecFlow.Table(new string[] {
                            "Customization",
                            "Value"});
                table2764.AddRow(new string[] {
                            "ELL - Each Accident / Total Policy / Each Employee",
                            "100,000/500,000/100,000"});
#line 56
 testRunner.When("user selects a plus plan quote with the following customizations from the WC your" +
                        " quote page:", ((string)(null)), table2764, "When ");
#line hidden
#line 59
 testRunner.Then("user begins the WC AI page setting the FEIN with value 83-2803691", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 60
 testRunner.Then("user continues on from the WC Additional Information page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2765 = new TechTalk.SpecFlow.Table(new string[] {
                            "Field",
                            "Value"});
                table2765.AddRow(new string[] {
                            "Payment Option",
                            "10% Down + 10 Monthly Installments"});
                table2765.AddRow(new string[] {
                            "CC Name",
                            "Funky Chicken-cheese"});
                table2765.AddRow(new string[] {
                            "CC Number",
                            "4111111111111111"});
                table2765.AddRow(new string[] {
                            "CC Expiration Month",
                            "11"});
                table2765.AddRow(new string[] {
                            "CC Expiration Year",
                            "25"});
                table2765.AddRow(new string[] {
                            "Autopay",
                            "No"});
                table2765.AddRow(new string[] {
                            "First Name",
                            "TestF"});
                table2765.AddRow(new string[] {
                            "Last Name",
                            "TestL"});
                table2765.AddRow(new string[] {
                            "Email",
                            "Test@Test123.com"});
                table2765.AddRow(new string[] {
                            "Phone",
                            "1231233333"});
                table2765.AddRow(new string[] {
                            "Same Billing Info?",
                            "Yes"});
#line 61
 testRunner.Then("user fills out the WC purchase page with these values:", ((string)(null)), table2765, "Then ");
#line hidden
#line 74
 testRunner.Then("user verifies the WC how would you rate our service modal", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 75
 testRunner.Then("user verifies the WC thank you page appearance", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
            }
            this.ScenarioCleanup();
        }
    }
}
#pragma warning restore
#endregion
