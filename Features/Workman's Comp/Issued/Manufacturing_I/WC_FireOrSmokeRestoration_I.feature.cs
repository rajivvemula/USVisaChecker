// ------------------------------------------------------------------------------
//  <auto-generated>
//      This code was generated by SpecFlow (https://www.specflow.org/).
//      SpecFlow Version:3.9.0.0
//      SpecFlow Generator Version:3.9.0.0
// 
//      Changes to this file may cause incorrect behavior and will be lost if
//      the code is regenerated.
//  </auto-generated>
// ------------------------------------------------------------------------------
#region Designer generated code
#pragma warning disable
namespace ApolloQAAutomation.Features.WorkmansComp.Issued.Manufacturing_I
{
    using TechTalk.SpecFlow;
    using System;
    using System.Linq;
    
    
    [System.CodeDom.Compiler.GeneratedCodeAttribute("TechTalk.SpecFlow", "3.9.0.0")]
    [System.Runtime.CompilerServices.CompilerGeneratedAttribute()]
    [Microsoft.VisualStudio.TestTools.UnitTesting.TestClassAttribute()]
    public partial class WC_FireOrSmokeRestoration_IFeature
    {
        
        private static TechTalk.SpecFlow.ITestRunner testRunner;
        
        private Microsoft.VisualStudio.TestTools.UnitTesting.TestContext _testContext;
        
        private static string[] featureTags = ((string[])(null));
        
#line 1 "WC_FireOrSmokeRestoration_I.feature"
#line hidden
        
        public virtual Microsoft.VisualStudio.TestTools.UnitTesting.TestContext TestContext
        {
            get
            {
                return this._testContext;
            }
            set
            {
                this._testContext = value;
            }
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.ClassInitializeAttribute()]
        public static void FeatureSetup(Microsoft.VisualStudio.TestTools.UnitTesting.TestContext testContext)
        {
            testRunner = TechTalk.SpecFlow.TestRunnerManager.GetTestRunner();
            TechTalk.SpecFlow.FeatureInfo featureInfo = new TechTalk.SpecFlow.FeatureInfo(new System.Globalization.CultureInfo("en-US"), "Features/Workman\'s Comp/Issued/Manufacturing_I", "WC_FireOrSmokeRestoration_I", @"Do you do any construction related activities including carpentry, painting, or handyman work? - yes
Do you do framing work (i.e. rough carpentry)? - no
Do you perform any roofing work? - no

'Do you do any roofing work?' may be rewritten as  'Do you perform any roofing work?' in the future.", ProgrammingLanguage.CSharp, featureTags);
            testRunner.OnFeatureStart(featureInfo);
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.ClassCleanupAttribute()]
        public static void FeatureTearDown()
        {
            testRunner.OnFeatureEnd();
            testRunner = null;
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestInitializeAttribute()]
        public void TestInitialize()
        {
            if (((testRunner.FeatureContext != null) 
                        && (testRunner.FeatureContext.FeatureInfo.Title != "WC_FireOrSmokeRestoration_I")))
            {
                global::ApolloQAAutomation.Features.WorkmansComp.Issued.Manufacturing_I.WC_FireOrSmokeRestoration_IFeature.FeatureSetup(null);
            }
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCleanupAttribute()]
        public void TestTearDown()
        {
            testRunner.OnScenarioEnd();
        }
        
        public void ScenarioInitialize(TechTalk.SpecFlow.ScenarioInfo scenarioInfo)
        {
            testRunner.OnScenarioInitialize(scenarioInfo);
            testRunner.ScenarioContext.ScenarioContainer.RegisterInstanceAs<Microsoft.VisualStudio.TestTools.UnitTesting.TestContext>(_testContext);
        }
        
        public void ScenarioStart()
        {
            testRunner.OnScenarioStart();
        }
        
        public void ScenarioCleanup()
        {
            testRunner.CollectScenarioErrors();
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestMethodAttribute()]
        [Microsoft.VisualStudio.TestTools.UnitTesting.DescriptionAttribute("WC Fire Or Smoke Damage Restoration is issued in Nevada")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestPropertyAttribute("FeatureTitle", "WC_FireOrSmokeRestoration_I")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("WC")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Manufacturing")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Regression")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Issued")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("NV")]
        public void WCFireOrSmokeDamageRestorationIsIssuedInNevada()
        {
            string[] tagsOfScenario = new string[] {
                    "WC",
                    "Manufacturing",
                    "Regression",
                    "Issued",
                    "NV"};
            System.Collections.Specialized.OrderedDictionary argumentsOfScenario = new System.Collections.Specialized.OrderedDictionary();
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("WC Fire Or Smoke Damage Restoration is issued in Nevada", null, tagsOfScenario, argumentsOfScenario, featureTags);
#line 9
this.ScenarioInitialize(scenarioInfo);
#line hidden
            if ((TagHelper.ContainsIgnoreTag(tagsOfScenario) || TagHelper.ContainsIgnoreTag(featureTags)))
            {
                testRunner.SkipScenario();
            }
            else
            {
                this.ScenarioStart();
                TechTalk.SpecFlow.Table table2397 = new TechTalk.SpecFlow.Table(new string[] {
                            "Industry",
                            "Employees",
                            "Location",
                            "Own or Lease",
                            "ZIP Code",
                            "LOB"});
                table2397.AddRow(new string[] {
                            "Fire or Smoke Damage Restoration",
                            "8",
                            "I Own a Property & Lease to Others",
                            "",
                            "89156",
                            "WC"});
#line 10
 testRunner.Given("user starts a quote with:", ((string)(null)), table2397, "Given ");
#line hidden
#line 13
 testRunner.Then("user verifies the appearance of the WC About You Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2398 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2398.AddRow(new string[] {
                            "When did you start your business?",
                            "Started 8 years ago"});
                table2398.AddRow(new string[] {
                            "How is your business structured?",
                            "Limited Liability Co. (LLC)"});
                table2398.AddRow(new string[] {
                            "What is the total annual payroll for W-2 employees? (exclude business owners/offi" +
                                "cers)",
                            "500000"});
                table2398.AddRow(new string[] {
                            "Do any employees only do general office work and rarely travel?",
                            "No"});
                table2398.AddRow(new string[] {
                            "Do you use any subcontractors or pay any workers using IRS Form 1099?",
                            "No"});
                table2398.AddRow(new string[] {
                            "Business",
                            ""});
                table2398.AddRow(new string[] {
                            "Address",
                            "105 Test Rd,Las Vegas"});
                table2398.AddRow(new string[] {
                            "Fill Contact",
                            ""});
#line 14
 testRunner.Then("user fills out the WC About You page with these values:", ((string)(null)), table2398, "Then ");
#line hidden
#line 24
 testRunner.Then("user verifies the appearance of the WC Owners and Officers Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2399 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2399.AddRow(new string[] {
                            "How many owners/officers does your business have?",
                            "3"});
                table2399.AddRow(new string[] {
                            "How many owners/officers do you want to cover with this policy?",
                            "3"});
                table2399.AddRow(new string[] {
                            "Do any included owners/officers only do general office work and rarely travel?",
                            "No"});
#line 25
 testRunner.Then("user handles the WC OO kickoff questions with these values:", ((string)(null)), table2399, "Then ");
#line hidden
#line 30
 testRunner.Then("user continues on from the WC OO page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 31
 testRunner.And("user verifies the appearance of the WC Your Services Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "And ");
#line hidden
                TechTalk.SpecFlow.Table table2400 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2400.AddRow(new string[] {
                            "Do you do any roofing work?",
                            "no"});
                table2400.AddRow(new string[] {
                            "Do you do asbestos abatement?",
                            "no"});
                table2400.AddRow(new string[] {
                            "Do you do any construction related activities including carpentry, painting, or h" +
                                "andyman work?",
                            "yes"});
                table2400.AddRow(new string[] {
                            "Do you do framing work (i.e. rough carpentry)?",
                            "no"});
                table2400.AddRow(new string[] {
                            "Do you perform any roofing work?",
                            "no"});
                table2400.AddRow(new string[] {
                            "In the past 3 years how many Workers\' Compensation claims were reported?",
                            "None"});
                table2400.AddRow(new string[] {
                            "Do you currently have a Workers\' Compensation insurance policy in effect?",
                            "yes"});
                table2400.AddRow(new string[] {
                            "Do you have multiple locations in more than one state?",
                            "no"});
                table2400.AddRow(new string[] {
                            "Federal Employer Identification Number (FEIN)",
                            "23-1626549"});
#line 32
 testRunner.Then("user fills out the WC Your Services page", ((string)(null)), table2400, "Then ");
#line hidden
#line 43
 testRunner.Then("user verifies the appearance of the WC Summary page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 44
    testRunner.Then("user clicks continue from WC Summary page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 45
    testRunner.Then("user verifies the WC your quote page appearance", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2401 = new TechTalk.SpecFlow.Table(new string[] {
                            "Customization",
                            "Value"});
                table2401.AddRow(new string[] {
                            "ELL - Each Accident / Total Policy / Each Employee",
                            "1,000,000/1,000,000/1,000,000"});
#line 46
 testRunner.When("user selects a plus plan quote with the following customizations from the WC your" +
                        " quote page:", ((string)(null)), table2401, "When ");
#line hidden
#line 49
 testRunner.Then("user begins the WC AI page having the FEIN with value 23-1626549", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 50
 testRunner.And("wc user handles 3 covered oo", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "And ");
#line hidden
#line 51
 testRunner.And("user continues on from the WC Additional Information page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "And ");
#line hidden
                TechTalk.SpecFlow.Table table2402 = new TechTalk.SpecFlow.Table(new string[] {
                            "Field",
                            "Value"});
                table2402.AddRow(new string[] {
                            "Payment Option",
                            "15% Down + 9 Monthly Installments"});
                table2402.AddRow(new string[] {
                            "CC Name",
                            "Boiler Inspector Guys"});
                table2402.AddRow(new string[] {
                            "CC Number",
                            "4111111111111111"});
                table2402.AddRow(new string[] {
                            "CC Expiration Month",
                            "11"});
                table2402.AddRow(new string[] {
                            "CC Expiration Year",
                            "25"});
                table2402.AddRow(new string[] {
                            "Autopay",
                            "N/A"});
                table2402.AddRow(new string[] {
                            "First Name",
                            "TestF"});
                table2402.AddRow(new string[] {
                            "Last Name",
                            "TestL"});
                table2402.AddRow(new string[] {
                            "Email",
                            "TestFTestL@Test123.com"});
                table2402.AddRow(new string[] {
                            "Phone",
                            "7777777777"});
                table2402.AddRow(new string[] {
                            "Same Billing Info?",
                            "Yes"});
#line 52
 testRunner.Then("user fills out the WC purchase page with these values:", ((string)(null)), table2402, "Then ");
#line hidden
#line 65
 testRunner.And("user verifies the WC how would you rate our service modal", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "And ");
#line hidden
#line 66
 testRunner.Then("user verifies the WC thank you page appearance", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
            }
            this.ScenarioCleanup();
        }
    }
}
#pragma warning restore
#endregion
