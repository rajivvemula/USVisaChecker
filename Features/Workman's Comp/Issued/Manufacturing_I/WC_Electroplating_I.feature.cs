// ------------------------------------------------------------------------------
//  <auto-generated>
//      This code was generated by SpecFlow (https://www.specflow.org/).
//      SpecFlow Version:3.9.0.0
//      SpecFlow Generator Version:3.9.0.0
// 
//      Changes to this file may cause incorrect behavior and will be lost if
//      the code is regenerated.
//  </auto-generated>
// ------------------------------------------------------------------------------
#region Designer generated code
#pragma warning disable
namespace ApolloQAAutomation.Features.WorkmansComp.Issued.Manufacturing_I
{
    using TechTalk.SpecFlow;
    using System;
    using System.Linq;
    
    
    [System.CodeDom.Compiler.GeneratedCodeAttribute("TechTalk.SpecFlow", "3.9.0.0")]
    [System.Runtime.CompilerServices.CompilerGeneratedAttribute()]
    [Microsoft.VisualStudio.TestTools.UnitTesting.TestClassAttribute()]
    public partial class WC_Electroplating_IFeature
    {
        
        private static TechTalk.SpecFlow.ITestRunner testRunner;
        
        private Microsoft.VisualStudio.TestTools.UnitTesting.TestContext _testContext;
        
        private static string[] featureTags = ((string[])(null));
        
#line 1 "WC_Electroplating_I.feature"
#line hidden
        
        public virtual Microsoft.VisualStudio.TestTools.UnitTesting.TestContext TestContext
        {
            get
            {
                return this._testContext;
            }
            set
            {
                this._testContext = value;
            }
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.ClassInitializeAttribute()]
        public static void FeatureSetup(Microsoft.VisualStudio.TestTools.UnitTesting.TestContext testContext)
        {
            testRunner = TechTalk.SpecFlow.TestRunnerManager.GetTestRunner();
            TechTalk.SpecFlow.FeatureInfo featureInfo = new TechTalk.SpecFlow.FeatureInfo(new System.Globalization.CultureInfo("en-US"), "Features/Workman\'s Comp/Issued/Manufacturing_I", "WC_Electroplating_I", "Issued Policy for Keyword: Electroplating\r\nEmployee option: 13\r\nZIP code: 52001 \r" +
                    "\nBusiness Structured: Corporation\r\nBusiness Operation: I Lease a Space From Othe" +
                    "rs\r\nBusiness started year: Started 8 years ago\r\nPayroll: 500,000\r\nIncluded owner" +
                    "s/officers: 2", ProgrammingLanguage.CSharp, featureTags);
            testRunner.OnFeatureStart(featureInfo);
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.ClassCleanupAttribute()]
        public static void FeatureTearDown()
        {
            testRunner.OnFeatureEnd();
            testRunner = null;
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestInitializeAttribute()]
        public void TestInitialize()
        {
            if (((testRunner.FeatureContext != null) 
                        && (testRunner.FeatureContext.FeatureInfo.Title != "WC_Electroplating_I")))
            {
                global::ApolloQAAutomation.Features.WorkmansComp.Issued.Manufacturing_I.WC_Electroplating_IFeature.FeatureSetup(null);
            }
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCleanupAttribute()]
        public void TestTearDown()
        {
            testRunner.OnScenarioEnd();
        }
        
        public void ScenarioInitialize(TechTalk.SpecFlow.ScenarioInfo scenarioInfo)
        {
            testRunner.OnScenarioInitialize(scenarioInfo);
            testRunner.ScenarioContext.ScenarioContainer.RegisterInstanceAs<Microsoft.VisualStudio.TestTools.UnitTesting.TestContext>(_testContext);
        }
        
        public void ScenarioStart()
        {
            testRunner.OnScenarioStart();
        }
        
        public void ScenarioCleanup()
        {
            testRunner.CollectScenarioErrors();
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestMethodAttribute()]
        [Microsoft.VisualStudio.TestTools.UnitTesting.DescriptionAttribute("WC Electroplating creates issued policy in Iowa")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestPropertyAttribute("FeatureTitle", "WC_Electroplating_I")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("WC")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Manufacturing")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Issued")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Regression")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("IA")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("YourServices")]
        public void WCElectroplatingCreatesIssuedPolicyInIowa()
        {
            string[] tagsOfScenario = new string[] {
                    "WC",
                    "Manufacturing",
                    "Issued",
                    "Regression",
                    "IA",
                    "YourServices"};
            System.Collections.Specialized.OrderedDictionary argumentsOfScenario = new System.Collections.Specialized.OrderedDictionary();
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("WC Electroplating creates issued policy in Iowa", null, tagsOfScenario, argumentsOfScenario, featureTags);
#line 12
this.ScenarioInitialize(scenarioInfo);
#line hidden
            if ((TagHelper.ContainsIgnoreTag(tagsOfScenario) || TagHelper.ContainsIgnoreTag(featureTags)))
            {
                testRunner.SkipScenario();
            }
            else
            {
                this.ScenarioStart();
                TechTalk.SpecFlow.Table table2389 = new TechTalk.SpecFlow.Table(new string[] {
                            "Industry",
                            "Employees",
                            "Location",
                            "Own or Lease",
                            "ZIP Code",
                            "LOB"});
                table2389.AddRow(new string[] {
                            "Electroplating",
                            "13",
                            "I Lease a Space From Others",
                            "",
                            "52001",
                            "WC"});
#line 13
 testRunner.Given("user starts a quote with:", ((string)(null)), table2389, "Given ");
#line hidden
#line 16
 testRunner.Then("user verifies the appearance of the WC About You Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2390 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2390.AddRow(new string[] {
                            "When do you want your policy to start?",
                            ""});
                table2390.AddRow(new string[] {
                            "When did you start your business?",
                            "Started 8 years ago"});
                table2390.AddRow(new string[] {
                            "How is your business structured?",
                            "Corporation"});
                table2390.AddRow(new string[] {
                            "What is the total annual payroll for W-2 employees? (exclude business owners/offi" +
                                "cers)",
                            "500000"});
                table2390.AddRow(new string[] {
                            "Are there any administrative or back office employees that are not involved at al" +
                                "l in the manufacturing process?",
                            "no"});
                table2390.AddRow(new string[] {
                            "Are there any travelling sales staff that are not involved at all in the manufact" +
                                "uring process?",
                            "no"});
                table2390.AddRow(new string[] {
                            "Are there any delivery drivers on staff?",
                            "no"});
                table2390.AddRow(new string[] {
                            "Business",
                            "Test Actor AK"});
                table2390.AddRow(new string[] {
                            "Address",
                            "350 W 6TH ST;Dubuque"});
                table2390.AddRow(new string[] {
                            "Contact First Name",
                            "TestF"});
                table2390.AddRow(new string[] {
                            "Contact Last Name",
                            "TestL"});
                table2390.AddRow(new string[] {
                            "Email",
                            "Test@Test123.com"});
                table2390.AddRow(new string[] {
                            "Phone",
                            "1231231321"});
                table2390.AddRow(new string[] {
                            "Business website",
                            "test.com"});
#line 17
 testRunner.Then("user fills out the WC About You page with these values:", ((string)(null)), table2390, "Then ");
#line hidden
#line 33
 testRunner.Then("user verifies the appearance of the WC Owners and Officers Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2391 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2391.AddRow(new string[] {
                            "How many owners/officers does your business have?",
                            "1"});
                table2391.AddRow(new string[] {
                            "How many owners/officers do you want to cover with this policy?",
                            "1"});
#line 34
 testRunner.Then("user handles the WC OO kickoff questions with these values:", ((string)(null)), table2391, "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2392 = new TechTalk.SpecFlow.Table(new string[] {
                            "First Name",
                            "Last Name",
                            "W2 payroll",
                            "Annual Payroll",
                            "Job Duty"});
                table2392.AddRow(new string[] {
                            "FirstOne",
                            "LastOne",
                            "yes",
                            "40000",
                            "Manufacturer"});
#line 38
 testRunner.Then("user handles the WC Covered OO with these values:", ((string)(null)), table2392, "Then ");
#line hidden
#line 41
 testRunner.Then("user continues on from the WC OO page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 42
 testRunner.Then("user verifies the appearance of the WC Your Services Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2393 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2393.AddRow(new string[] {
                            "Other than hand-held power tools, do you use any equipment or machinery?",
                            "yes"});
                table2393.AddRow(new string[] {
                            "When equipment/machinery is out of service do you place a sign or a lock that it " +
                                "should not be used?",
                            "yes"});
                table2393.AddRow(new string[] {
                            "Do your employees deliver any of your finished product?",
                            "Sometimes (<50% of the time)"});
                table2393.AddRow(new string[] {
                            "In the past 3 years how many Workers\' Compensation claims were reported?",
                            "None"});
                table2393.AddRow(new string[] {
                            "Are any employees required to physically lift/move more than 50 lbs?",
                            "no"});
                table2393.AddRow(new string[] {
                            "Do you currently have a Workers\' Compensation insurance policy in effect?",
                            "no"});
                table2393.AddRow(new string[] {
                            "When was your last policy in effect?",
                            "Never no prior insurance"});
                table2393.AddRow(new string[] {
                            "Do you handle, store, or transport any hazardous corrosive substances?",
                            "no"});
                table2393.AddRow(new string[] {
                            "Do you handle, store, or transport any explosives or blasting agents?",
                            "no"});
                table2393.AddRow(new string[] {
                            "Do you handle, store, or transport any ammonium nitrate?",
                            "no"});
                table2393.AddRow(new string[] {
                            "Do you have multiple locations in more than one state?",
                            "no"});
#line 43
 testRunner.Then("user fills out the WC Your Services page", ((string)(null)), table2393, "Then ");
#line hidden
#line 56
 testRunner.Then("user verifies the appearance of the WC Summary page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 57
 testRunner.Then("user clicks continue from WC Summary page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2394 = new TechTalk.SpecFlow.Table(new string[] {
                            "Customization",
                            "Value"});
                table2394.AddRow(new string[] {
                            "ELL - Each Accident / Total Policy / Each Employee",
                            "100,000/500,000/100,000"});
#line 58
 testRunner.When("user selects a standard multi-bundle plan quote with the following customizations" +
                        " from the WC your quote page:", ((string)(null)), table2394, "When ");
#line hidden
#line 61
 testRunner.Then("user begins the WC AI page setting the FEIN with value 06-2671064", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2395 = new TechTalk.SpecFlow.Table(new string[] {
                            "Have Name"});
                table2395.AddRow(new string[] {
                            ""});
#line 62
 testRunner.Then("wc user handles 1 excluded oo with these values:", ((string)(null)), table2395, "Then ");
#line hidden
#line 65
 testRunner.Then("user continues on from the WC Additional Information page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2396 = new TechTalk.SpecFlow.Table(new string[] {
                            "Field",
                            "Value"});
                table2396.AddRow(new string[] {
                            "Payment Option",
                            "One Pay Plan"});
                table2396.AddRow(new string[] {
                            "CC Name",
                            "Celestial Echos"});
                table2396.AddRow(new string[] {
                            "CC Number",
                            "4111111111111111"});
                table2396.AddRow(new string[] {
                            "CC Expiration Month",
                            "11"});
                table2396.AddRow(new string[] {
                            "CC Expiration Year",
                            "25"});
                table2396.AddRow(new string[] {
                            "Autopay",
                            "No"});
                table2396.AddRow(new string[] {
                            "First Name",
                            "TestF"});
                table2396.AddRow(new string[] {
                            "Last Name",
                            "TestL"});
                table2396.AddRow(new string[] {
                            "Email",
                            "TestFTestL@Test123.com"});
                table2396.AddRow(new string[] {
                            "Phone",
                            "7777777777"});
                table2396.AddRow(new string[] {
                            "Same Billing Info?",
                            "Yes"});
#line 66
 testRunner.Then("user fills out the WC purchase page with these values:", ((string)(null)), table2396, "Then ");
#line hidden
#line 79
 testRunner.Then("user verifies the WC how would you rate our service modal", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 80
 testRunner.Then("user verifies the WC thank you page appearance", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
            }
            this.ScenarioCleanup();
        }
    }
}
#pragma warning restore
#endregion
