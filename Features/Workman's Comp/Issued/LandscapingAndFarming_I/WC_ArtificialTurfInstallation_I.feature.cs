// ------------------------------------------------------------------------------
//  <auto-generated>
//      This code was generated by SpecFlow (https://www.specflow.org/).
//      SpecFlow Version:3.9.0.0
//      SpecFlow Generator Version:3.9.0.0
// 
//      Changes to this file may cause incorrect behavior and will be lost if
//      the code is regenerated.
//  </auto-generated>
// ------------------------------------------------------------------------------
#region Designer generated code
#pragma warning disable
namespace ApolloQAAutomation.Features.WorkmansComp.Issued.LandscapingAndFarming_I
{
    using TechTalk.SpecFlow;
    using System;
    using System.Linq;
    
    
    [System.CodeDom.Compiler.GeneratedCodeAttribute("TechTalk.SpecFlow", "3.9.0.0")]
    [System.Runtime.CompilerServices.CompilerGeneratedAttribute()]
    [Microsoft.VisualStudio.TestTools.UnitTesting.TestClassAttribute()]
    public partial class WC_ArtificialTurfInstallation_IFeature
    {
        
        private static TechTalk.SpecFlow.ITestRunner testRunner;
        
        private Microsoft.VisualStudio.TestTools.UnitTesting.TestContext _testContext;
        
        private static string[] featureTags = ((string[])(null));
        
#line 1 "WC_ArtificialTurfInstallation_I.feature"
#line hidden
        
        public virtual Microsoft.VisualStudio.TestTools.UnitTesting.TestContext TestContext
        {
            get
            {
                return this._testContext;
            }
            set
            {
                this._testContext = value;
            }
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.ClassInitializeAttribute()]
        public static void FeatureSetup(Microsoft.VisualStudio.TestTools.UnitTesting.TestContext testContext)
        {
            testRunner = TechTalk.SpecFlow.TestRunnerManager.GetTestRunner();
            TechTalk.SpecFlow.FeatureInfo featureInfo = new TechTalk.SpecFlow.FeatureInfo(new System.Globalization.CultureInfo("en-US"), "Features/Workman\'s Comp/Issued/LandscapingAndFarming_I", "WC_ArtificialTurfInstallation_I", " Issuing a Artificial Turf Installation Policy in Iowa", ProgrammingLanguage.CSharp, featureTags);
            testRunner.OnFeatureStart(featureInfo);
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.ClassCleanupAttribute()]
        public static void FeatureTearDown()
        {
            testRunner.OnFeatureEnd();
            testRunner = null;
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestInitializeAttribute()]
        public void TestInitialize()
        {
            if (((testRunner.FeatureContext != null) 
                        && (testRunner.FeatureContext.FeatureInfo.Title != "WC_ArtificialTurfInstallation_I")))
            {
                global::ApolloQAAutomation.Features.WorkmansComp.Issued.LandscapingAndFarming_I.WC_ArtificialTurfInstallation_IFeature.FeatureSetup(null);
            }
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCleanupAttribute()]
        public void TestTearDown()
        {
            testRunner.OnScenarioEnd();
        }
        
        public void ScenarioInitialize(TechTalk.SpecFlow.ScenarioInfo scenarioInfo)
        {
            testRunner.OnScenarioInitialize(scenarioInfo);
            testRunner.ScenarioContext.ScenarioContainer.RegisterInstanceAs<Microsoft.VisualStudio.TestTools.UnitTesting.TestContext>(_testContext);
        }
        
        public void ScenarioStart()
        {
            testRunner.OnScenarioStart();
        }
        
        public void ScenarioCleanup()
        {
            testRunner.CollectScenarioErrors();
        }
        
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestMethodAttribute()]
        [Microsoft.VisualStudio.TestTools.UnitTesting.DescriptionAttribute("Artificial Turf Installation- IA")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestPropertyAttribute("FeatureTitle", "WC_ArtificialTurfInstallation_I")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Regression")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("YourServices")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("IA")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("WC")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Landscaping")]
        [Microsoft.VisualStudio.TestTools.UnitTesting.TestCategoryAttribute("Issued")]
        public void ArtificialTurfInstallation_IA()
        {
            string[] tagsOfScenario = new string[] {
                    "Regression",
                    "YourServices",
                    "IA",
                    "WC",
                    "Landscaping",
                    "Issued"};
            System.Collections.Specialized.OrderedDictionary argumentsOfScenario = new System.Collections.Specialized.OrderedDictionary();
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("Artificial Turf Installation- IA", null, tagsOfScenario, argumentsOfScenario, featureTags);
#line 6
this.ScenarioInitialize(scenarioInfo);
#line hidden
            if ((TagHelper.ContainsIgnoreTag(tagsOfScenario) || TagHelper.ContainsIgnoreTag(featureTags)))
            {
                testRunner.SkipScenario();
            }
            else
            {
                this.ScenarioStart();
                TechTalk.SpecFlow.Table table2246 = new TechTalk.SpecFlow.Table(new string[] {
                            "Industry",
                            "Employees",
                            "Location",
                            "Own or Lease",
                            "ZIP Code",
                            "LOB"});
                table2246.AddRow(new string[] {
                            "Artificial Turf Installation",
                            "2",
                            "I Lease a Space From Others",
                            "",
                            "50613",
                            "WC"});
#line 7
 testRunner.Given("user starts a quote with:", ((string)(null)), table2246, "Given ");
#line hidden
#line 10
 testRunner.Then("user verifies the appearance of the WC About You Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2247 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2247.AddRow(new string[] {
                            "When do you want your policy to start?",
                            ""});
                table2247.AddRow(new string[] {
                            "When did you start your business?",
                            "Started 9 years ago"});
                table2247.AddRow(new string[] {
                            "How is your business structured?",
                            "Corporation"});
                table2247.AddRow(new string[] {
                            "What is the total annual payroll for W-2 employees? (exclude business owners/offi" +
                                "cers)",
                            "440440"});
                table2247.AddRow(new string[] {
                            "Do any employees only do general office work and rarely travel?",
                            "no"});
                table2247.AddRow(new string[] {
                            "Do you use any subcontractors or pay any workers using IRS Form 1099?",
                            "no"});
                table2247.AddRow(new string[] {
                            "Business",
                            "Artificial Turf Installation"});
                table2247.AddRow(new string[] {
                            "Address",
                            "1310 W 1st St;Cedar Falls"});
                table2247.AddRow(new string[] {
                            "Fill Contact",
                            ""});
#line 11
 testRunner.Then("user fills out the WC About You page with these values:", ((string)(null)), table2247, "Then ");
#line hidden
#line 22
 testRunner.Then("user verifies the appearance of the WC Owners and Officers Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2248 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2248.AddRow(new string[] {
                            "How many owners/officers does your business have?",
                            "2"});
                table2248.AddRow(new string[] {
                            "How many owners/officers do you want to cover with this policy?",
                            "1"});
#line 23
 testRunner.Then("user handles the WC OO kickoff questions with these values:", ((string)(null)), table2248, "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2249 = new TechTalk.SpecFlow.Table(new string[] {
                            "First Name",
                            "Last Name",
                            "W2 Payroll",
                            "Annual Payroll",
                            "Job Duty"});
                table2249.AddRow(new string[] {
                            "Ken",
                            "Masters",
                            "yes",
                            "50000",
                            "Physical Laborer"});
#line 27
 testRunner.Then("user handles the WC Covered OO with these values:", ((string)(null)), table2249, "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2250 = new TechTalk.SpecFlow.Table(new string[] {
                            "First Name",
                            "Last Name"});
                table2250.AddRow(new string[] {
                            "Cammy",
                            "White"});
#line 30
 testRunner.Then("user handles the WC Excluded OO with these values:", ((string)(null)), table2250, "Then ");
#line hidden
#line 33
 testRunner.Then("user continues on from the WC OO page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 34
 testRunner.Then("user verifies the appearance of the WC Your Services Page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2251 = new TechTalk.SpecFlow.Table(new string[] {
                            "Question",
                            "Answer"});
                table2251.AddRow(new string[] {
                            "Do you trim trees above ground level or remove trees/stumps?",
                            "no"});
                table2251.AddRow(new string[] {
                            "Do you do any new landscaping such as laying down bricks,  sod, or stones or plan" +
                                "ting work?",
                            "no"});
                table2251.AddRow(new string[] {
                            "Do you do any roofing work?",
                            "no"});
                table2251.AddRow(new string[] {
                            "Do you ever transport six or more workers in the same vehicle?",
                            "no"});
                table2251.AddRow(new string[] {
                            "Do you do any framing work?",
                            "yes"});
                table2251.AddRow(new string[] {
                            "What percentage of work done is framing (rough  carpentry)?",
                            "40"});
                table2251.AddRow(new string[] {
                            "Do you provide any lot or land clearing services?",
                            "yes"});
                table2251.AddRow(new string[] {
                            "Do you do any construction work such as carpentry, decking, handyman jobs, or pai" +
                                "nting?",
                            "yes"});
                table2251.AddRow(new string[] {
                            "Do you do any logging work?",
                            "no"});
                table2251.AddRow(new string[] {
                            "In the past 3 years how many Workers\' Compensation claims were reported?",
                            "None"});
                table2251.AddRow(new string[] {
                            "Do you currently have a Workers\' Compensation insurance policy in effect?",
                            "yes"});
                table2251.AddRow(new string[] {
                            "Do you have multiple locations in more than one state?",
                            "no"});
                table2251.AddRow(new string[] {
                            "Social Security Number (SSN)",
                            "231-11-1111"});
#line 35
 testRunner.Then("user fills out the WC Your Services page", ((string)(null)), table2251, "Then ");
#line hidden
#line 50
 testRunner.Then("user verifies the appearance of the WC Summary page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 51
    testRunner.Then("user clicks continue from WC Summary page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 52
    testRunner.Then("user verifies the WC your quote page appearance", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2252 = new TechTalk.SpecFlow.Table(new string[] {
                            "Customization",
                            "Value"});
                table2252.AddRow(new string[] {
                            "ELL - Each Accident / Total Policy / Each Employee",
                            "100,000/500,000/100,000"});
#line 53
 testRunner.When("user selects a standard multi-bundle plan quote with the following customizations" +
                        " from the WC your quote page:", ((string)(null)), table2252, "When ");
#line hidden
#line 56
 testRunner.Then("user verifies the appearance of the WC Additional Information page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 57
 testRunner.Then("user begins the WC AI page having the SSN with value 231-11-1111", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 58
 testRunner.Then("user continues on from the WC Additional Information page", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
                TechTalk.SpecFlow.Table table2253 = new TechTalk.SpecFlow.Table(new string[] {
                            "Field",
                            "Value"});
                table2253.AddRow(new string[] {
                            "Payment Option",
                            "15% Down + 9 Monthly Installments"});
                table2253.AddRow(new string[] {
                            "CC Name",
                            "Radiatoin Safety Services"});
                table2253.AddRow(new string[] {
                            "CC Number",
                            "4111111111111111"});
                table2253.AddRow(new string[] {
                            "CC Expiration Month",
                            "11"});
                table2253.AddRow(new string[] {
                            "CC Expiration Year",
                            "25"});
                table2253.AddRow(new string[] {
                            "Autopay",
                            "No"});
                table2253.AddRow(new string[] {
                            "First Name",
                            "Funky"});
                table2253.AddRow(new string[] {
                            "Last Name",
                            "Kong"});
                table2253.AddRow(new string[] {
                            "Email",
                            "FunkyKong@Cheese.com"});
                table2253.AddRow(new string[] {
                            "Phone",
                            "7777777777"});
                table2253.AddRow(new string[] {
                            "Same Billing Info?",
                            "Yes"});
#line 59
 testRunner.Then("user fills out the WC purchase page with these values:", ((string)(null)), table2253, "Then ");
#line hidden
#line 72
 testRunner.Then("user verifies the WC how would you rate our service modal", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 73
 testRunner.Then("user verifies the WC thank you page appearance", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
#line 74
 testRunner.Then("user verifies that these LOBs are recommended: GL", ((string)(null)), ((TechTalk.SpecFlow.Table)(null)), "Then ");
#line hidden
            }
            this.ScenarioCleanup();
        }
    }
}
#pragma warning restore
#endregion
