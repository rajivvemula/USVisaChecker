Feature: PL_EmergencyMedicalTechnician(EMS)_I

Issuing a PL Policy using the Emergency Medical Technician (EMT) keyword

@Regression @PL @Health @Issued @ND
Scenario: PL Emergency Medical Technician (EMT) issued policy in ND
	Given user starts a quote with:
		| Industry                           | Employees | Location                    | Own or Lease | ZIP Code | LOB |
		| Emergency Medical Technician (EMT) | 2         | I Lease a Space From Others |              | 58011    | PL  |
	...
	Then user fills out A Quick Introduction page with these values:
		...
		| Corporation        | EMS              | 407 Main St      | No  |
