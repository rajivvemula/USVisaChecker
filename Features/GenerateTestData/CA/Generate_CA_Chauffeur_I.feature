Feature: Generate_CA_Chauffeur_I

@Generate @CA
Scenario: CA_Chauffeur successfully purchases a policy for Indiana
	Given user starts a quote with:
		| Industry  | Employees | Location                              | Own or Lease | ZIP Code | LOB |
		| Chauffeur | 2         | I Run My Business From Property I Own | Vehicles     | 92130    | CA  |
	...
	And user fills in the Introduction page with these values:
		...
		| 1900                  | Individual/Sole Proprietor | 5327 Morning Sage Way |          | San Diego | Original          |
	...
	And user creates a vehicle or trailer with variable values:
		| VIN               | Type Insure                     | Parking Address | Vehicle Worth | Seatbelt |
		| 2L1MJ5LK0FBL00196 | Limousine: seating less than 10 | California      | 12000         | Yes      |
	Then user adds another vehicle or trailer
	And user creates a vehicle or trailer with variable values:
		| VIN               | Type Insure | Parking Address | Vehicle Worth |
		| 1L1FM88W15Y654453 | Car - Sedan | California      | 10000         |
	Then user adds another vehicle or trailer
	And user creates a vehicle or trailer with variable values:
		| VIN               | Parking Address | Vehicle Worth |
		| 2HGFC1E57KH701257 | California      | 10000         |
	Then user adds another vehicle or trailer
	And user creates a vehicle or trailer with variable values:
		| VIN               | Parking Address |
		| KNADM4A34G6664425 | California      |
	Then user adds another vehicle or trailer
	And user creates a vehicle or trailer with variable values:
		| VIN               | Parking Address |
		| 2C4RC1AGXLR135979 | California      |
	Then user clicks Let's Continue
	Then User verifies appearance of the Drivers Page
	And user creates a driver with these values:
		| FirstName | LastName | DLState | DOB        | CDL | ILDDC | ILViolation | ILDLRevoked | Accident | DLNumber |
		| Donald    | Duck     | CA      | 03/13/1986 |     |       |             |             | No       | A0002144 |
		| Goofy     | Woofy    | IN      | 04/01/1988 |     |       |             |             | No       |          |
		| Mike      | Jones    | IN      | 09/03/1989 |     |       |             |             | No       |          |
		| Jon       | Jones    | IN      | 06/03/1958 |     |       |             |             | No       |          |
		| Chris     | Jones    | IN      | 11/28/1998 |     |       |             |             | No       |          |
	...
	Then user fills out the Operations page:
		...
		| Do you have or plan on applying for an operating authority from the California Public Utilities Commission (PUC)? | No                              |
