Feature: Generate_CA_Handyman_I

Populate the system with testing data primarily base on most common policy types per book of business.

@Generate @CA
Scenario: Generate CA Handyman successfully purchases a policy for CA
	Given user starts a quote with:
		| Industry               | Employees | Location                              | Own or Lease | ZIP Code | LOB |
		| Construction: Handyman | 2         | I Run My Business From Property I Own | Vehicles     | 92123    | CA  |
	...
	And user fills in the Introduction page with these values:
		...
		| 1900                  | Individual/Sole Proprietor | 3249 Towser St |          | San Diego |                   | Yes             |
	Then user clicks continue from CA Introduction
	Then user verifies the appearance of the Vehicles page
	And user creates a vehicle or trailer with variable values:
		| VIN               | Parking Address | Vehicle Worth | Plowing Snow | Escort Trucks |
		| 3D3KR19D97G838924 | California      | 12000         | No           | No            |
	Then user adds another vehicle or trailer
	And user creates a vehicle or trailer with variable values:
		| VIN               | Parking Address | Plowing Snow | Escort Trucks |
		| 5NMZU3LB2JH065041 | California      | No           | No            |
	Then user adds another vehicle or trailer
	And user creates a vehicle or trailer with variable values:
		| VIN               | Parking Address | Vehicle Worth | Plowing Snow | Escort Trucks |
		| 1FTNE2EW0DDA17671 | California      | 10000         | No           | No            |
	Then user clicks Let's Continue
	Then User verifies appearance of the Drivers Page
	And user creates a driver with these values:
		| FirstName | LastName | DLState | DOB        | CDL | ILDDC | ILViolation | ILDLRevoked | Accident | DLNumber |
		| Chris     | Rock     | CA      | 03/13/1986 |     |       |             |             | No       | A0002143 |
		| Eric      | Smith    | CA      | 04/01/1988 |     |       |             |             | No       | A0002142 |
		| Mike      | Jones    | CA      | 09/03/1989 |     |       |             |             | No       | A0002141 |
	...
	And user enters contact information:
		...
		| First Name                      | Ariana                 |
		| Last Name                       | Grande                 |
		| Business Email                  | Handyman@yopmail.com   |
		...
		| Owner's Address                 | 3249 Towser St         |
		| Owner's Address 2               |                        |
		| Owner's Zip Code                | 92123                  |
		| Owner's City                    | San Diego              |
